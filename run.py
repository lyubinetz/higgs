...
def run(verbose_training):
  ...
  '''
  print('Started the run - it is expected to take around 1 hour!')
  try:
  ...
  for idx in range(NUM_NETS):
    ...
    nn.fit(Xt, y_train[indices], verbose=verbose_training, num_iters=4500, learning_rate=0.01, update_strategy='rmsprop',
